...
See detr/models/matcher.py for additional information.
"""
import torch
    ...
    def forward(self, outputs, targets):
        ...
        Params:
            outputs: This is a dict that contains at least these entries:
                 ...
                 "pred_coords": Tensor of dim [batch_size, num_queries, 2] with the predicted box coordinates

            targets: This is a list of targets (len(targets) = batch_size), where each target is a dict containing:
                 "labels": Tensor of dim [num_target_coords] (where num_target_coords is the number of ground-truth
                           objects in the target) containing the class labels
                 "coords": Tensor of dim [num_target_coords, 2] containing the target box coordinates
        ...
        """
        bs, num_queries = outputs["pred_node_logits"].shape[:2]

        # We flatten to compute the cost matrices in a batch
        out_prob = outputs["pred_node_logits"].flatten(0, 1).softmax(-1)  # [batch_size * num_queries, num_classes]
        out_coord = outputs["pred_coords"].flatten(0, 1)  # [batch_size * num_queries, 2]
...
def build_matcher(args):
    return HungarianMatcher(cost_class=args.set_cost_class, cost_coord=args.set_cost_coord)
