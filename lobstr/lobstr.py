"""
LOBSTR model and criterion classes.
...
class LOBSTR(nn.Module):
    """ This is the LOBSTR module that performs graph recognition """
    def __init__(self, backbone, transformer, num_node_classes, num_edge_classes, num_queries, aux_loss=False):
        ...
        self.edge_class_embed = nn.Linear(hidden_dim, num_queries * num_edge_classes)
    ...
    def forward(self, samples: NestedTensor):
        ...
        outputs_node_class = self.node_class_embed(hs)
        outputs_edge_class = self.edge_class_embed(hs)
        outputs_coord = self.bbox_embed(hs).sigmoid()
        out = {'pred_node_logits': outputs_node_class[-1],
               'pred_edge_logits': outputs_edge_class[-1],
               'pred_boxes': outputs_coord[-1]}
        if self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(outputs_node_class, outputs_coord)
        return out

    @torch.jit.unused
    def _set_aux_loss(self, outputs_node_class, outputs_edge_class,
                      outputs_coord):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        return [{'pred_node_logits': a,
                 'pred_edge_logits': b,
                 'pred_boxes': c}
                for a, b, c in zip(outputs_node_class[:-1],
                                   outputs_edge_class[:-1],
                                   outputs_coord[:-1])]
...
class SetCriterion(nn.Module):
    ...
    def loss_adjacencies(self, outputs, targets, indices, num_boxes):
        ...
        """
        assert 'pred_edge_logits' in outputs
        ...
        for batch_num in range(batch_size):
            ...
            src_adjacencies = outputs['pred_edge_logits'][batch_num]
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        """
        out_logits, out_bbox = outputs['pred_node_logits'], outputs['pred_boxes']
...
def build(args):
    num_classes=args.num_node_classes
